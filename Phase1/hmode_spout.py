...
class WordSpout(Spout):
    ...
    def initialize(self, stormconf, context):
        self.in_path = '//home/ahmadpour/Documents/src/Intern_Web_Crawler/csv_files/hmode.csv'
        self.url_pd = pd.read_csv(self.in_path)
        self.solr_conn = pysolr.Solr("http://localhost:8983/solr/PAcked_Core",always_commit=True, timeout=100)
        self.solr_prd_conn = pysolr.Solr("http://localhost:8983/solr/Product_Core",always_commit=True, timeout=100)
        self.solr_update_conn = pysolr.Solr("http://localhost:8983/solr/Update-Core",always_commit=True, timeout=100)
        self.business_name = 'Hmode' #***
        self.string_query = 'business_name:' +'"' + self.business_name + '"'   #***
